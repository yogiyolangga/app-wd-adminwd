...
const Data = ({ loading, apiUrl, dataHistory, setLoading, getDataHistory }) => {
  ...
  const handleClickUndo = async (id) => {
    try {
      await new Promise((resolve) => setTimeout(resolve, 300));
      const response = await Axios.put(`${apiUrl}/adminwd/undoclosed`, {
        id,
      });
      if (response.data.success) {
        getDataHistory();
                      ...
                      <div className={`flex justify-center gap-1`}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (confirm) {
                              handleClickUndo(item.closed_id);
                            }
                          }}
                        >
                          <IoArrowUndoCircleSharp className="text-zinc-50" />
                        </button>
                      </div>
                    </td>
                  </tr>
